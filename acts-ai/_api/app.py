...
def f(query, history=""):
    system_content = "You are a licensed therapist and are giving advice to a patient. After 10 messages recommend a therapist to the user. Provide their name and website to find them."
    stream = False
    ...
    async def generate(request: Request):
        ...
        def gen():
            ...
            for segment in f(body["input"], body["history"]):
                yield {
                    "type": "text",
                    "value": segment.choices[0].message.content,
